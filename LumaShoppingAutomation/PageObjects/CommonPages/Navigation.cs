...
namespace LumaShoppingAutomation.PageObjects.CommonPages
{
    public class Navigation
    {
        ...
        PageElement Button(string text, int index = 1) => new PageElement(_Driver, By.XPath($"(//*[not(count(self::span) > 1) and translate(normalize-space(text()),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')=\"{text.ToLower()}\"]//ancestor::*[(self::button or self::a or @onclick or @role='button') and not(contains(@class,'banner'))])[{index}]"));
